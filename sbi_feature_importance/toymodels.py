...
from sbi.samplers.mcmc.init_strategy import proposal_init, sir
...
from sbi_feature_importance.utils import sample_posterior_potential
...
class GM:
    ...
    def sample_posterior_potential(
        ...
        if init_strategy == "sir":
            init_func = lambda: sir(self.Prior, potential_fn, transform=transform)
...
class GLM(GM):
    """Gaussian Lineaer Model (GLM) for testing Simulation Based Inference (SBI).

    Implements sampling and log_prob functionality
    for the joint probability distribution, likelihood, and
    posterior distributions of a GLM with parameters:
    ...
    def __init__(self, prior: Optional[Distribution] = None, seed: int = None):
        if prior is None:
    ...
    def model_parameters(
        glm, theta: Tensor, xdims2keep: List[int] = [0, 1, 2, 3], sigma_sq: float = 0.1
        ...
        Args:
            ...
            sigma_sq: Variance of the GLM.
        Returns:
            m: Mean of GLM.
            S: Covariance matrix of GLM.
            L: Linear Mapping applied to theta.
            mu_x: Translational parameter.
        """
        ...
        mu_x, L = glm.default_LM(in_dims, k_samples, xdims2keep)

        # LM
        m = mu_x + theta @ L
        m = m.view(k_samples, 1, out_dims)
        S = sigma_sq * torch.eye((out_dims)).repeat(k_samples, 1, 1)

        return m, S

    @classmethod
    def posterior_glm_params(
        glm, prior: Distribution, context: Tensor, xdims: List[int] = [0, 1, 2, 3]
        ...
        pdim = theta.shape[-1]
        m, S = glm.model_parameters(torch.zeros_like(theta), xdims2keep=xdims)
        mu_x, L = glm.default_LM(pdim, m.shape[0], xdims)
        sigma_sq = S[0, 0, 0]
    ...
    def log_prob_posterior(
        ...
        """Evaluate the posterior of the GLM for a set of obervations.
        Args:
        ...
        Returns:
            log_prob: logarithmic probabilities of each sample.
        """
        mean, cov, support = self.posterior_glm_params(
    ...
    def rejection_sampling(
        glm, mean: Tensor, cov: Tensor, support: Tensor, k_samples: int,
    ) -> Tensor:
        ...
        sample_cache = []
        while k_samples != 0:
            sample = glm.cholesky_sample(mean, cov, k_samples)
            within_support = glm.is_supported(support, sample).view(-1)
    ...
    def sample_posterior_within_prior(
        glm, k_samples: int, prior: Distribution, context: Tensor, xdims: List[int]
    ) -> Tensor:
        """Draws/redraws k samples until all are within their prior support.
        ...
        """
        mean, cov, support = glm.posterior_glm_params(prior, context, xdims)

        # sample while rejecting samples outside of prior support
        sample = glm.rejection_sampling(mean, cov, support, k_samples)
...
# def torchIO(func: Callable, *args, **kwargs):
#     def wrapper(*args, **kwargs):
#         args = [arg.numpy() if type(arg) == torch.Tensor else arg for arg in args]
#         kwargs = {x: y.numpy() if type(y) == torch.Tensor else x for (x,y) in zip(kwargs.keys(), kwargs.values())}
#         outputs = func(*args, **kwargs)
#         if type(outputs) == tuple:
#             converted_outputs = tuple(torch.tensor(x) for x in outputs if type(x) == np.ndarray)
#         elif type(outputs) == np.ndarray:
#             converted_outputs = torch.tensor(outputs)
...
class SimpleHH(HHSimulator):
    ...
    def simulate(
        self, theta: Tensor, n_workers: int = 12, batch_size: int = 50, seed: int = 0
    ) -> Tensor:
