...
from numpy import histogram2d, linspace, size
...
from sbi_feature_importance.metrics import cov, sample_kl, unbiased_mmd_squared
from sbi_feature_importance.snle import ReducablePosterior
from sbi_feature_importance.utils import (feature_stats, ints_from_str,
                                          select_tag, skip_dims,
...
def plot_iqrchanges(
    ...
    im = ax.imshow(IQRs_agg.numpy(), cmap=cmap, norm=LogNorm(vmin=zlims[0], vmax=zlims[1]),)
    ...
    **plot_kwargs,
    ...
    def set_plot_params(t, timewindow, style, figsize, line_colors, line_widths):
        ...
        if len(line_colors) < n_samples:
            line_colors = line_colors + [line_colors[-1]] * (n_samples - len(line_colors))
        if len(line_widths) < n_samples:
            line_widths = line_widths + [line_widths[-1]] * (n_samples - len(line_widths))
