...
# ephys extraction
import ephys_helper.extractor as efex
import ephys_helper.features as ft
from ephys_helper.utils import constant_stimulus, sigmoid
...
class HHSimulator:
    ...
    Attributes:
        ...
        model_params: All parameters that are static for the HH simulator rather than dynamic.
            ...
            T_1 [°C] :  36 °C from paper MartinPospischil et al.
    ...
    def set_static_parameters(
        ...
        3. T_1 [°C] :  36 °C from paper MartinPospischil et al.
        4. T_2 [°C] :  Experimental temperature.
        5. Q10 : temperature coeff.
        6. tau [ms] : Membrane time constant.
        7. input_res [Mohm] : Membrane input resistance.
        8. C [uF/cm^2] : Membrane capcitance per area.
        9. gNa [mS] : Channel conductance of sodium.
        10. gK [mS] : Channel conductance of potassium.
        11. gM [mS] : Channel conductance for adpt. K currents.
        12. gLeak [mS] : Leak conductance.
        13. gL [mS] : Channel conductance for Calcium current.
        14. tau_max [s] :
        15. VT [mV] : Threshold voltage.
        16. Eleak  [mV] : Reversal potential of leak currents.
        17. rate_to_SS_factor : Correction factor.
    ...
    def dict_init(self, dct: Dict):
        ...
        Args:
            dct: Dictionary with key value pairs for parameters. Dict needs to contain
                ...
                ["ENa", "EK", "ECa", "T_1", "T_2", "Q10", "tau", "input_res",
                "C", "gNa", "gK", "gM", "gLeak", "gL", "tau_max", "VT",
                "Eleak","rate_to_SS_factor"]

                OR

                [r"$E_{Na}$", r"$E_{K}$", r"$E_{Ca}$", "T_1",
                "T_2", "Q10", r"$\tau$", r"$R_{in}$",
                r"C", r"$g_{Na}$", r"$g_{K}$",r"$g_{M}$",
                r"$g_{leak}$", r"$g_{L}$", r"$\tau_{max}$",
                r"$V_{T}$", r"$E_{leak}$", r"$r_{SS}$"]
    ...
    def summarise(
        ...
        for dt, t, V, I in zip(dts, ts, Vs, Is):
            ...
            AHP_2ndhalf = float("nan")
            AP_threshold_2ndhalf = float("nan")
            AP_amplitude_2ndhalf = float("nan")
            AP_width_2ndhalf = float("nan")
            # UDR_3 = float("nan")
...
def HH_Br2(
    ...
    eqs = """
            dVm/dt = -(gNa*m**3*h*(Vm - ENa) + gK*n**4*(Vm - EK) + gleak*(Vm - Eleak) + gM*p*(Vm - EK) 
                    + gL*q**2*r*(Vm - ECa) - I_inj)/ (C * area) : volt
            I_inj = I(t, i)*pA : amp
            ...
            dq/dt = (alphaq*(1-q) - betaq*q) * t_adj_factor : 1
            dr/dt = (alphar*(1-r) - betar*r) * t_adj_factor : 1
            ...
            betah = 4.0/(1 + exp((-(Vm - VT - 40.0*mV)) / (5.0*mV)))/ms : Hz

            alphan = (-0.032/mV) * (Vm - VT - 15.0*mV) / (exp((-(Vm - VT - 15.0*mV)) / (5.0*mV)) - 1.0)/ms : Hz
            betan = 0.5*exp(-(Vm - VT - 10.0*mV) / (40.0*mV))/ms : Hz

            p_inf = 1.0/(1.0 + exp(-(Vm + 35.0*mV)/(10.0*mV))) : 1
            tau_p = (tau_max/1000.0)/(3.3 * exp((Vm + 35.0*mV)/(20.0*mV)) + exp(-(Vm + 35.0*mV)/(20.0*mV))) : second

            alphaq = (0.055/mV) * (-27.0*mV - Vm) / (exp((-27.0*mV - Vm)/(3.8*mV)) - 1.0)/ms : Hz
            betaq = 0.94*exp((-75.0*mV - Vm) / (17.0*mV))/ms : Hz

            alphar = 0.000457*exp((-13.0*mV - Vm)/(50.0*mV))/ms : Hz
            betar = 0.0065/(1 + exp((-15.0*mV - Vm)/(28.0*mV)))/ms : Hz
    ...
    T_1 = model_params[:, 3]  # °C, from paper MartinPospischil et al.
    T_2 = model_params[:, 4]  # °C, experiment was actually done at 34 °C
    Q10 = model_params[:, 5]  # temperature coeff.

    tau = model_params[:, 6]
    input_res = model_params[:, 7]
    C_ = model_params[:, 8]
    ...
    neurons.gNa = model_params[:, 9] * br2.mS / br2.cm ** 2 * area_
    neurons.gK = model_params[:, 10] * br2.mS / br2.cm ** 2 * area_
    neurons.gM = model_params[:, 11] * br2.mS / br2.cm ** 2 * area_
    neurons.gleak = model_params[:, 12] * br2.mS / br2.cm ** 2 * area_
    neurons.gL = model_params[:, 13] * br2.mS / br2.cm ** 2 * area_
    neurons.tau_max = model_params[:, 14] * br2.second
    neurons.VT = model_params[:, 15] * br2.mV
    neurons.Eleak = model_params[:, 16] * br2.mV
    neurons.rate_to_SS_factor = model_params[:, 17]
    ...
    if V0 != None:
        ...
        neurons.Vm = model_params[:, 18] * br2.mV  # V0
