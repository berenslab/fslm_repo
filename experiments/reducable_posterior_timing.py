...
if main_args.featuresets == "random":
    get_subsets = partial(random_subsets_of_dims, seed=4)
...
if "max_len" in main_args.policy_kwargs:
    method_tag += f"_{eval(main_args.policy_kwargs)['max_len']}"
if "min_len" in main_args.policy_kwargs:
    method_tag += f"_{eval(main_args.policy_kwargs)['min_len']}"

task_queue = []
if main_args.policy == "default":
    for dims in [all_dims] + get_subsets(all_dims)[:2]:
